...
(function() {
  ...
  Ball = (function(_super) {
    ...
    Ball.prototype.initializeDimensions_ = function() {
      ...
      if (SPECIAL_SHAPE !== 0) {
        this.radius_ = Config.MAX_BALL_RADIUS * 2.3;
      }
...
(function() {
  ...
  Box = (function(_super) {
    ...
    Box.prototype.initializeDimensions_ = function() {
      ...
      if (SPECIAL_SHAPE !== 0) {
        this.width_ = Config.MAX_BOX_DIMENSION * 2.3;
        this.height_ = Config.MAX_BOX_DIMENSION * 2.3;
      }
...
(function() {
  ...
  WashingMachine = (function() {
    ...
    WashingMachine.prototype.goCrazy_ = function() {
      if (Math.random() < .001) {
        crazyOn = !crazyOn;
      }

      if (!crazyOn) {
        Config.SIMULATION_RATE = 0.5;
        return;
      }
      ...
      if (flash > 1) {
        document.querySelector("#main-canvas").style.webkitTransform = 'rotateZ(' + (Math.random() > 0.5 ? '-' : '') + Math.floor(360 * Math.min(1, flash / 100)) + 'deg)';
      }

      if (!colorsOff && flash - (Math.random() * 900) > 0) {
        r = Math.min(255, Math.max(0, r + (Math.random() > 0.5 ? 1 : -1)));
        g = Math.min(255, Math.max(0, g + (Math.random() > 0.5 ? 1 : -1)));
        b = Math.min(255, Math.max(0, b + (Math.random() > 0.5 ? 1 : -1)));
        ...
        if (Math.random() < 0.1) {
          ...
          if (circle.style.fill !== '#f209a0') {
            if (circle.style.fill === '#000000') {
              ...
              circle.style.cssText = 'fill: #000';
            }
          }
        }

        if (flash - (Math.random() * 900) > 500) {
          flash = 0;
        }
      } else {
      ...
      Config.SIMULATION_RATE = (colorsOff ? 0.5 : 0.1) * rateScalar;

      if (rateScalar === 0) {
        if (freeze > 80 + (Math.random() * 30)) {
          freeze = 0;
          rateScalar = 1;
        } else {
          freeze += 1;
        }
      } else if (rateScalar === 8) {
        if (freeze > 12 + (Math.random() * 40)) {
          freeze = 0;
          rateScalar = 1;
        } else {
          freeze += 1;
